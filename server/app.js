  ...
  try {
    ...
    task_data.reminders.forEach((ele, i) => {
      ...
      scheduleJob(`${task_id}_${i}`, ele, () => {
        if (i < 3) {
          console.log(
            ` Hi ${userFound.firstname} This is a reminder ${
              i + 1
            } for completing your task ${task_data.task_name}`
          );
          console.log(new Date());
        } else {
          console.log(
            ` Hi ${userFound.firstname} ,your deadline for ${task_data.task_name} has been passed`
          );
        }
      });
    });
...
app.get("/api/task/:task_id", async (req, res) => {
  try {
    ...
    let tasks_of_task_ID = userFound.tasks.find(
      (ele) => ele.task_id == task_id
    );
...
app.delete("/api/task/:task_id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        success: "Task  Deleted and Reminders have been cancelled Successfully",
      });
  } catch (error) {
    console.error(error);
